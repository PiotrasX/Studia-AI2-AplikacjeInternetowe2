...
async function fetchTrips() {
    try {
        ...
        if (selectedCountry.value) params.append('country_name', selectedCountry.value)
        const res = await fetch(`http://localhost:3000/trips?${params.toString()}`)
        ...
        if (selectedCountry.value) countParams.append('country_name', selectedCountry.value)
        const allRes = await fetch(`http://localhost:3000/trips?${countParams.toString()}`)
        const allData = await allRes.json()
        if (!allRes.ok) throw new Error(allData.error || allData.message || 'Błąd podczas pobierania wycieczek.')
...
async function fetchCountries() {
    try {
        let url = 'http://localhost:3000/countries?limit=9999'
        if (selectedContinent.value) {
            url += `&continent_name=${encodeURIComponent(selectedContinent.value)}`
        }
        ...
        const tripsRes = await fetch('http://localhost:3000/trips?limit=9999')
        ...
        const countriesWithTrips = new Set(tripsList.map(trip => trip.country_name))
        const filteredCountries = allCountries.map(c => c.name).filter(name => countriesWithTrips.has(name))

        countries.value = filteredCountries.sort()
    } catch (err) {
        if (err instanceof TypeError && err.message === "Failed to fetch") toastStore.show("Serwer nie odpowiada, spróbuj ponownie później.", "error")
        else toastStore.show(err.message, "error")
    }
...
// Przeniesienie do strony dodawania wycieczki
function goToAddTrip() {
    router.push('/trips/add')
}
...
async function downloadPDF() {
    try {
        ...
        if (selectedContinent.value) params.append('continent_name', selectedContinent.value)
        if (selectedCountry.value) params.append('country_name', selectedCountry.value)
        const res = await fetch(`http://localhost:3000/trips/report/pdf?${params.toString()}`, {
            headers: {
        ...
        a.href = url;
        a.download = "trips_report.pdf";
        a.click();
        window.URL.revokeObjectURL(url);

        toastStore.show("Raport PDF dla wycieczek pobrany.", "success");
    } catch (err) {
		if (err instanceof TypeError && err.message === "Failed to fetch") toastStore.show("Serwer nie odpowiada, spróbuj ponownie później.", "error")
		else toastStore.show(err.message, "error")
    } finally {
    ...
    <div class="trips">
        ...
        <section class="hero">
            <div class="hero-header">
                <h1 class="hero-title">Nasze wycieczki</h1>
            </div>
            <p class="hero-subtitle">
                Odkryj świat z nami – od egzotycznych plaż po majestatyczne góry.
                Wybierz kierunek, który rozbudzi Twoją ciekawość!
            </p>
            <div class="hero-admin-btn">
                <button v-if="isAdmin" @click="goToAddTrip" class="add-btn">Dodaj wycieczkę</button>
        ...
        <section class="filters">
            <div class="search-div">
                <input v-model="search" type="text" placeholder="Szukaj wycieczki, kontynentu lub kraju..."
                    class="filter-input" />
            </div>
            ...
            <div class="filters-div">
                ...
                <select v-model="selectedCountry" class="filter-select" :disabled="!countries.length">
                    <option value="">Wszystkie kraje</option>
                    <option v-for="c in countries" :key="c" :value="c">{{ c }}</option>
                </select>

                <select v-model="limit" @change="changeLimit" class="filter-select">
                    <option value="5">5 wycieczek na stronę</option>
                    <option value="10">10 wycieczek na stronę</option>
                    <option value="20">20 wycieczek na stronę</option>
                    <option value="30">30 wycieczek na stronę</option>
                    <option value="50">50 wycieczek na stronę</option>
                </select>
            </div>
        </section>
        ...
        <section class="table-section">
            <TripsTable :trips="trips" :loading="loading" :sort="sort" :order="order" @sort="toggleSort" />
        </section>

        <!-- Paginacja -->
        <section class="pagination" v-if="!loading && trips.length">
