...
async function fetchUsers() {
	try {
		...
		if (selectedRole.value) countParams.append('role', selectedRole.value)
		const allRes = await fetch(`http://localhost:3000/users?${countParams.toString()}`, {
			headers: {
				'Content-Type': 'application/json',
				...getAuthHeader()
			}
		...
		if (!allRes.ok) throw new Error(allData.error || allData.message || 'Błąd podczas pobierania użytkowników.')
...
// Przeniesienie do strony dodawania użytkownika
...
async function downloadPDF() {
    try {
		...
		if (selectedRole.value) params.append('role', selectedRole.value)
        const res = await fetch(`http://localhost:3000/users/report/pdf?${params.toString()}`, {
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeader()
            }
        ...
        toastStore.show("Raport PDF dla użytkowników pobrany.", "success");
    } catch (err) {
		if (err instanceof TypeError && err.message === "Failed to fetch") toastStore.show("Serwer nie odpowiada, spróbuj ponownie później.", "error")
		else toastStore.show(err.message, "error")
    } finally {
	...
	<div class="users">
		...
		<section class="hero">
			<div class="hero-header">
				<h1 class="hero-title">Użytkownicy systemu</h1>
			</div>
			<p class="hero-subtitle">
				Przeglądaj konta użytkowników – wyszukuj, sortuj i sprawdzaj szczegóły profili.
			</p>
			<div class="hero-admin-btn">
				<button v-if="isAdmin" @click="goToAddUser" class="add-btn">Dodaj użytkownika</button>
		...
		<section class="filters">
			<div class="search-div">
				<input v-model="search" type="text" placeholder="Szukaj po imieniu, nazwisku lub adresie e-mail..."
					class="filter-input" />
			</div>
			...
			<div class="filters-div">
				...
				<select v-model="limit" @change="changeLimit" class="filter-select">
					<option value="5">5 użytkowników na stronę</option>
					<option value="10">10 użytkowników na stronę</option>
					<option value="20">20 użytkowników na stronę</option>
					<option value="30">30 użytkowników na stronę</option>
					<option value="50">50 użytkowników na stronę</option>
				</select>
			</div>
		</section>
		...
		<section class="table-section">
			<UsersTable :users="users" :loading="loading" :sort="sort" :order="order" @sort="toggleSort" />
		</section>

		<!-- Paginacja -->
		<section class="pagination" v-if="!loading && users.length">
